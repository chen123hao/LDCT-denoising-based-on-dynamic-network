    ...
    for p_ind, patient in enumerate(patients_list):
        ...
        for path_ in [patient_input_path, patient_target_path]:
            ...
            for pi in range(len(full_pixels)):
                ...
                f_name = '{}_{}_{}.npy'.format(patient, pi+15, io)
...
def load_scan(path):
    ...
    slices = [pydicom.read_file(os.path.join(path, s)) for s in os.listdir(path)]###########force=True
    slices.sort(key=lambda x: float(x.ImagePositionPatient[2]))##如果有的图像缺失部分meta info，则无法获取ImagePositionPatient。此时可以注释掉第二行
    try:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_path', type=str, default='./AAPM/')
    parser.add_argument('--save_path', type=str, default='./3npy_img_eg/')
