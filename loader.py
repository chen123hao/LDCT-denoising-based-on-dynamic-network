...
class ct_dataset(Dataset):
    def __init__(self, mode, load_mode, saved_path, test_patient, patch_n=None, patch_size=None, transform=None):
        ...
        if mode == 'train':
            input_path = sorted(glob(os.path.join(saved_path, 'train','*_input.npy')))
            target_path = sorted(glob(os.path.join(saved_path, 'train','*_target.npy')))
            input_ = [f for f in input_path ]#input_ = [f for f in input_path if test_patient  in f]
            target_ = [f for f in target_path ]#target_ = [f for f in target_path if test_patient  in f]
            ...
            input_path = sorted(glob(os.path.join(saved_path, 'test','*_input.npy')))
            target_path = sorted(glob(os.path.join(saved_path, 'test','*_target.npy')))
            input_ = [f for f in input_path]  # input_ = [f for f in input_path if test_patient  in f]
            target_ = [f for f in target_path]  # target_ = [f for f in target_path if test_patient  in f]
            if load_mode == 0:
                self.input_ = input_
